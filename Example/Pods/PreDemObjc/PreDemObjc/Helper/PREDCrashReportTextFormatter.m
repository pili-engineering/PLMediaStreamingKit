//
//  PREDCrashReportTextFormatter.m
//  PreDemObjc
//
 ...
 */
@implementation PREDCrashReportTextFormatter

NSString *const PREDXamarinStackTraceDelimiter = @"Xamarin Exception Stack:";
...
+ (PREDBinaryImageType)pres_imageTypeForImagePath:(NSString *)imagePath processPath:(NSString *)processPath {
    if (!imagePath || !processPath) {
        return PREDBinaryImageTypeOther;
    }
    PREDBinaryImageType imageType = PREDBinaryImageTypeOther;
...
+ (NSString *)pres_archNameFromImageInfo:(PREPLCrashReportBinaryImageInfo *)imageInfo
{
    ...
    if (imageInfo.codeType != nil && imageInfo.codeType.typeEncoding == PLCrashReportProcessorTypeEncodingMach) {
        archName = [PREDCrashReportTextFormatter pres_archNameFromCPUType:imageInfo.codeType.type subType:imageInfo.codeType.subtype];
    }
    ...
    if (frameInfo.symbolInfo != nil && imageType != PREDBinaryImageTypeOther) {
        ...
        if ([symbolName rangeOfString: @"_"].location == 0 && [symbolName length] > 1) {
            switch (report.systemInfo.operatingSystem) {
                ...
                default:
                    PREDLogDebug(@"Symbol prefix rules are unknown for this OS!");
                    break;
            }
        }
...
+ (NSString *)anonymizedPathFromPath:(NSString *)path {
    ...
    if (([path length] > 0) && [path hasPrefix:@"/Users/"]) {
        ...
        if (error) {
            PREDLogError(@"String replacing failed - %@", error.localizedDescription);
        }
    }
